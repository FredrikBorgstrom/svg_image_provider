...
class SvgImageProvider extends ImageProvider<SvgImageProvider> {
  ...
  const SvgImageProvider(this.assetName,
      {this.scale = 1.0, this.containerSize});
  ...
  Future<ImageInfo> _loadImage(SvgImageProvider key) async {
    try {
      ...
      print(
          'Rendering at: $scaledWidth x $scaledHeight (device scale: $deviceScale)');
      assert(scaledWidth > 0 && scaledHeight > 0,
          'Invalid target size: $scaledWidth x $scaledHeight');

      final recorder = PictureRecorder();
      final canvas = Canvas(recorder,
          Rect.fromLTWH(0, 0, scaledWidth.toDouble(), scaledHeight.toDouble()));
      canvas.transform(Matrix4.diagonal3Values(
              scaledWidth / size.width, scaledHeight / size.height, 1.0)
          .storage);
